...
seed_cache <- function(layerdef, initial_view, crs, warp_opts = .warp_opts, resampling_method = .resampling_method, zoom_levels = 1, cache) {
    ...
    for (zoom in sort(zoom_levels)) {
        ...
        for (z in raster_layers) {
            for (i in seq_len(nrow(imgdef$xy_grid))) cache_fetch_tile(imgdef, z = z, i = i, layerdef = layerdef, crs = crs, warp_opts = warp_opts, resampling_method = resampling_method, cache) ## ids
        }
    }
}
...
cache_fetch_tile <- function(imgdef, z, i, layerdef, crs, warp_opts, resampling_method, cache) {
